...
function StatisticsPage() {
  ...
  return (
    <MobileLayout>
      {/* 전체를 감싸는 컨테이너에 배경색 적용하여 상단까지 통일 */}
      <div className="flex flex-col h-full bg-[#fff5f0]">
        
        {/* 헤더 & 상단 탭 */}
        {/* ⚠️ 주한님 요청 반영: bg-white를 제거하고 전체 배경색과 통일함 */}
        ...
        <main className="flex-1 overflow-y-auto pb-24 px-5 pt-4">
          ...
          <div className="space-y-4">
              ...
              <div className="bg-white shadow-sm border border-purple-100/30 rounded-2xl p-4 flex flex-col justify-between min-h-[140px]">
                ...
                <div className="space-y-2">
                  {musicRecommendations.map((music) => (
                    <div key={music.music_recommendation_id} className="flex items-center gap-1.5 text-xs text-gray-700 group cursor-pointer">
            ...
            <div className="bg-white shadow-sm border border-emerald-100/30 rounded-2xl p-4">
              ...
              <p className="text-xs text-gray-700 font-semibold leading-relaxed">"{missionRecommendation.mission_content}"</p>
            </div>
          </div>
        </main>
      </div>
